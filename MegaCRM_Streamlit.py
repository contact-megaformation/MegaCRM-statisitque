...
if not df_all.empty and "DateAjout_dt" in df_all.columns:
    ...
    if month_pick:
        ...
        daily_clients_map = df_all_dates[df_all_dates["DateAjout_dt"].dt.date == _today].groupby("__sheet_name")["Nom & Prénom"].count()
        daily_inscrits_map = df_all_dates[
...
if role == "موظف" and employee:
    ...
    if options:
        ...
        with st.form(f"edit_client_form::{employee}"):
            ...
            with col2:
                ...
                new_birth = st.date_input("🎂 تاريخ الميلاد", value=default_birth)
                ...
                new_insc = st.selectbox(
                    "🟢 التسجيل",
                    ["Pas encore", "Inscrit"],
                    index=1 if str(cur_row.get("Inscription","")).strip().lower() in ("oui","inscrit") else 0,
                )
        ...
        if submitted:
            try:
                ...
                col_map = {h: (EXPECTED_HEADERS.index(h) + 1) for h in [
                    "Nom & Prénom","Téléphone","Date de naissance","Formation","Date ajout","Date de suivi","Inscription","Remarque"
                ]}
    ...
    def _s(x): return str(x).strip() if x is not None else ""
    def _nonempty(x): return bool(_s(x))
    ...
    def build_admin_report_for_date(df_source: pd.DataFrame, employee_name: str, target_date: date) -> str:
        ...
        d["Inscription_norm"] = d.get("Inscription", "").fillna("").astype(str).str.strip().str.lower()

        added_rows = d[d["DateAjout_dt"].dt.date == target_date].copy()
        contacts_rows = d[d["DateSuivi_dt"].dt.date == target_date].copy()
        alerts_rows = contacts_rows[contacts_rows["Alerte_eff"].apply(_nonempty)].copy()
        ...
        lines.append(f"👤 الموظّف: {employee_name}")
        lines.append("")
        ...
        if contacts_rows.empty:
            ...
            for _, r in contacts_rows.sort_values("Nom & Prénom", na_position="last").iterrows():
                name = _s(r.get("Nom & Prénom", ""))
                phone = _s(r.get("Téléphone", ""))
                form = _s(r.get("Formation", ""))
                t_contact = _s(r.get("Type de contact", ""))
                note = _s(r.get("Remarque", ""))
                alert_txt = _s(r.get("Alerte_eff", ""))
        ...
        if alerts_rows.empty:
            ...
            for _, r in alerts_rows.sort_values("Nom & Prénom", na_position="last").iterrows():
                ...
                lines.append(
                    f"- {name} ({form}) — {phone}\n"
                    f"  🚨 {alert_txt}\n"
                    f"  📝 ملاحظات: {note if note else '(لا يوجد)'}"
                )
    ...
    try:
        ...
        st.markdown(f"[📲 إرسال التقرير عبر WhatsApp]({url})")
...
if tab_choice == "أرشيف" and role == "موظف" and employee:
    ...
    if df_emp_all.empty:
        ...
        move_opt = st.selectbox(
            ...
            [f"{r['Nom & Prénom']} — {format_display_phone(r['Téléphone'])}" for _, r in df_emp_all.iterrows()],
        )
        if st.button("📦 نقل إلى الأرشيف"):
            try:
                ...
                tel_idx = header.index("Téléphone") if "Téléphone" in header else header.index("Téléphonique")
    ...
    if df_arch.empty:
        ...
        restore_opt = st.selectbox(
            "اختر عميل للاسترجاع",
            [f"{r['Nom & Prénom']} — {format_display_phone(r['Téléphone'])}" for _, r in df_arch.iterrows()],
            key="restore_pick",
        )
